...
# Function: scratch implementation of kaplan-meier survival estimation add propensity...
#######################################################################################
...
def fit_weighted_km(T, E, treatment, propensity):
  """
  Fit Kaplan-Meier suvival curve estimator weighting by propensity score.

  Parameters:
    T: array-like, time to event
    E: array-like, event indicator, assumes 0 means censored, 1 means event
    treatment: array-like, treatment indicator, assumes 0 means control, 1 means treatment
    propensity: array-like, propensity scores
  """
  # Sort data by time to event, add survival column
  df = pd.DataFrame({'T': T, 'E': E, 
                     'treatment':treatment,
                     'propensity':propensity}).sort_values(by='T')
  df['S'] = np.nan
  ...
  for t in df['T'].unique():
    ...
    # Compute denominator: number of people at risk
    d = ((df['T'] >= t)*df[(df['T'] >= t)]['weight']).sum()

    # Compute numerator: number of individuals with event at t
    n = (((df['T'] == t) & (df['E'] == 1))*df[(df['T'] == t) & (df['E'] == 1)]['weight']).sum()
